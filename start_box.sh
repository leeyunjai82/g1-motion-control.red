#!/usr/bin/env bash
# G1 통합 서비스 실행 스크립트
# 위치: /home/circulus/project/g1-motion-control/start.sh
# 사용: ./start.sh
...
# 관리 대상 스크립트
TARGETS=("rs_stream.py" "ik_box.py" "dashboard.py")
...
cleanup() {
  ...
  # 최대 8초 대기 (ik_box의 종료 시퀀스 ~5.5초 + 여유)
...
python -u rs_stream.py > "$LOG_DIR/rs_stream.log" 2>&1 &
PIDS+=($!); NAMES+=("rs_stream")
sleep 2

echo "[start] ik_box     (50000) ..."
python -u ik_box.py    > "$LOG_DIR/ik_box.log"    2>&1 &
PIDS+=($!); NAMES+=("ik_box")
sleep 3
