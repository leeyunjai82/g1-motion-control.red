...
# 로그 토글
# ==========================================
VERBOSE_LOG = False   # True = [BOX] 로그 출력, False = 조용히


MIN_PIXELS    = 100
MIN_DEPTH_M   = 0.10
MAX_DEPTH_M   = 3.00
TOP_THRESH_M  = 0.005   # 윗면 ±5mm
...
class BoxEstimator:
    ...
    def detect(self, color_bgr, depth_mm, gravity_cam=None):
        ...
        is_top = np.abs(h - h_top) < TOP_THRESH_M
        if is_top.sum() < 30:
            return out
        ...
        # === 4 꼭지점: approxPolyDP로 polygon 단순화 ===
        # 박스가 회전해도 진짜 모서리 4개 추출
        # epsilon = 외곽선 둘레의 2% (점진적으로 조정)
        contour_pts = contour.reshape(-1, 2).astype(np.float32)
        perimeter = cv2.arcLength(contour, True)

        corners_4 = None
        for eps_ratio in [0.02, 0.03, 0.04, 0.05, 0.015, 0.06, 0.08]:
        ...
        if corners_4 is None:
            # approxPolyDP가 4점 못 찾음 → fallback: 화면 극단점
            x = contour_pts[:, 0]
            y = contour_pts[:, 1]
            TL = contour_pts[np.argmin(x + y)]
            TR = contour_pts[np.argmax(x - y)]
            BL = contour_pts[np.argmin(x - y)]
            BR = contour_pts[np.argmax(x + y)]
        ...
        if VERBOSE_LOG:
            ...
            print(f"[BOX] h_top={h_top*100:+.1f}cm "
